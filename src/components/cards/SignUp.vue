<template>
  <form @submit.prevent="signUp" class="w-full h-full p-3 md:p-5">
    ...
    <button class="w-full py-2 bg-pink text-title text-white rounded-md" @click="logIn">
      Log in
    </button>
  </form>
</template>
...
export default {
  ...
  emits: ['login', 'signedUp'],
  methods: {
    signUp() {
      this.$apiPost('/register', this.user).then((res: any) => {
        this.$emit('signedUp', res, false)
      }).catch((err: any) => {
        this.$emit('signedUp', err, true)
      })
    },
    logIn() {
      this.$emit('login')
